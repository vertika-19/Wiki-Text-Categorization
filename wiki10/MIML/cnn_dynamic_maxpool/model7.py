...
class Model7:
    def __init__(self,maxParagraphs,paragraphLength,labels,vocabularySize,filterSizes,num_filters\
        ,wordEmbeddingDimension,lrate,poolLength):
        ...
        self.fullyConnectedLayerInput = int(len(self.filterSizes)*self.num_filters*( \
        ...
        # #gpu_options = tf.GPUOptions(device_count = {'GPU':0})
    ...
    def convLayer(self):
        ...
        return tf.reshape(filtered_outputs,[-1,self.fullyConnectedLayerInput])
    ...
    def fullyConnectedLayer(self):
        ...
        #adding dropout
        self.layer_dropout = tf.nn.dropout( self.convOutput , 0.5 )

        # layer = tf.nn.sigmoid(tf.matmul(self.convOutput, weights) + bias)
        layer = tf.nn.sigmoid(tf.matmul(self.layer_dropout, weights) + bias)
        return layer
